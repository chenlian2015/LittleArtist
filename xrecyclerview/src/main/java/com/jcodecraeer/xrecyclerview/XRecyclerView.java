...
public class XRecyclerView extends RecyclerView {
    ...
    private void init(Context context) {
        ...
        if (pullRefreshEnabled) {
            ...
            mRefreshHeader.setProgressStyle(ProgressStyle.EJiaZi);
        }
        LoadingMoreFooter footView = new LoadingMoreFooter(mContext);
        //        footView.setProgressStyle(mLoadingMoreProgressStyle);
        footView.setProgressStyle(ProgressStyle.EJiaZi);
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            Log.i("xrecyclerView",layoutManager.getChildCount()+"  "+lastVisibleItemPosition+"  "+layoutManager.getItemCount());
    ...
    //2016.4.6 添加，解决fragment复用时列表拉倒最下面后，再更新数据时，无法加载更多的问题
    public void cleanState() {
        isnomore = false;
        previousTotal = 0;
        View footView = mFootViews.get(0);
        if (footView instanceof LoadingMoreFooter) {
            ((LoadingMoreFooter) footView).setState(LoadingMoreFooter.STATE_CLEAN);
        }
    }
